...
function Navbar() {
    ...
    return (
        <>
            <div className={`w-full  ${isMinimized ? 'h-14' : 'h-20'} fixed top-0 z-50 flex items-center ${darkMode ? 'bg-dark-blue-dark-mode-elements text-white duration-100' : 'bg-white text-very-dark-blue-light-mode-text duration-500'} px-4 sm:px-8 md:px-24 shadow-lg transition-all ease-in-out`}>
                
                <div className="w-full h-10 flex items-center justify-between">

                    <div className="w-48 sm:w-60 h-full flex items-center relative">
                        <img src={Logo} alt="" className={`${isMinimized ? 'h-10 sm:h-12' : 'h-12 sm:h-14'} mr-4 transition-all duration-300 ease-in-out`} />
